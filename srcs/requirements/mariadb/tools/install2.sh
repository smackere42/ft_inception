...
EOF
	...
	if [ "$MYSQL_DATABASE" != "" ]; then
		...
		if [ "$MYSQL_CHARSET" != "" ] && [ "$MYSQL_COLLATION" != "" ]; then
			...
			echo "CREATE DATABASE IF NOT EXISTS \`$MYSQL_NAME\` CHARACTER SET $MYSQL_CHARSET COLLATE $MYSQL_COLLATION;" >> $tfile
		else
			echo "[i] with character set: 'utf8' and collation: 'utf8_general_ci'"
			echo "CREATE DATABASE IF NOT EXISTS \`$MYSQL_NAME\` CHARACTER SET utf8 COLLATE utf8_general_ci;" >> $tfile
		fi

		if [ "$MYSQL_USER" != "" ]; then
			echo "[i] Creating user: $MYSQL_USER with password $MYSQL_PASSWORD"
			echo "GRANT ALL ON \`$MYSQL_NAME\`.* to '$MYSQL_USER'@'%' IDENTIFIED BY '$MYSQL_PASSWORD';" >> $tfile
		fi
	fi
	...
	echo "exec /bin/mysqld --user=mysql --console --skip-name-resolve --skip-networking=0" "$@"
fi

exec /bin/ --user=mysql --console --skip-name-resolve --skip-networking=0 $@